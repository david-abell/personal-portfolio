...
This project was a scripting testbed and exploration of performance impacts encountered while importing large CSV files (13M records) into a SQLite database. I tested Popular CSV parsing libraries [PapaParse](https://www.papaparse.com) and [csv-parse for Node.js](https://csv.js.org/parse/) using Node stream events as well as using the modern code style using an async `for await (...)` iterator with `pipeline`. I also tested a few SQL PRAGMA settings with PapaParse that ending up having significant performance benefits.

## Testing CSV import performance

### csv-parse

Parsing with csv-parse was simple to set up [link-to-file](https://github.com/david-abell/gtfs-to-sqlite/blob/ce4644e5d1573e82caf4263dcdab5b760f368740/importFileCsvEvents.ts) by following the excellent stream documentation [here](https://csv.js.org/parse/api/stream/).
...
Lines are formatted during each event and pushed to an array for batch database insertion. I also tested the built in `cast` option but calling a custom casting function with this option proved more than twice as slow than just performing type conversion as part of the formatLine step.
...
Pipeline with a for await loop greatly simplifies code complexity when working with streams by not requiring direct management of event listeners. I had high hopes for the below snippet.
...
While the above code looks simple compared to events, it was not up to the task. PapaParse [_code_here_](https://github.com/david-abell/gtfs-to-sqlite/blob/ce4644e5d1573e82caf4263dcdab5b760f368740/importPipeline.ts#L1) took 467 seconds compared to just 65 seconds need by the event based code above.

And this still doesn't include database inserts!

This article by Dan Vanderkam [here](https://medium.com/netscape/async-iterators-these-promises-are-killing-my-performance-4767df03d85b) goes into the problem in more detail. The short version is that creating a promise for each line entry causes far too much overhead.

Performance has been improving in node though. This on going discussion [Performance of for await of (async iteration)](https://github.com/nodejs/node/issues/31979) indicates there is still more performance left on the table.
...
PapaParse takes 54 seconds to parse 13M records when only logging the completion of each csv file. Batching these logs to near SQLite's variable count limit increased parse time further to 65 seconds. Logging every event took 2446 seconds...40.76 minutes of my time wasted for science.

### SQLite Pragma

#### Performance of common PRAGMA settings on 330k records

With a smaller 330k record dataset, disabling journal mode and synchronous were the only PRAGMA with noticeable impacts.
...
With the full 13M dataset, journal mode and synchronous offered significant performance benefits. Increasing cache size had a less noticeable but still significant effect. Changes to locking mode and temp store had no effect.
...
Negative cache_size=N setting reflects the number of pages that would use approximately abs(N\*1024) bytes
Positive cache_size=N sets cache size to N
